...
class Store:
    ...
    def normalise_prefix(self, prefix):
        if prefix is None:
            return ""
    ...
    def merge(self, other, prefix=None):
        new_prefix = self.normalise_prefix(prefix)
        for path, commands in other.paths.items():
            for name, options in commands.items():
                self.paths[path][f"{new_prefix}{name}"] = options
        ...
        class command_spec(sb.Spec):
            ...
            def normalise_filled(self, meta, val):
                ...
                if path not in self.paths:
                    raise BadSpecValue("Unknown path", wanted=path, available=sorted(self.paths), meta=meta.at("path"))
